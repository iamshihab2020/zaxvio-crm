...
export default async function quoteRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const { jobPipelineStages, pipelines: pipelinesTable } = await import("@hvac-saas/database");
      const defaultPipeline = await db
        .select({ id: pipelinesTable.id })
        .from(pipelinesTable)
        .where(and(eq(pipelinesTable.tenantId, tenantId), eq(pipelinesTable.isDefault, true)))
        .then((r) => r[0]);
