...
export default async function publicBookingRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/:slug/availability", async (request, reply) => {
    ...
    if (!month || !/^\d{4}-\d{2}$/.test(month)) {
      return reply.status(400).send({ message: "Invalid month format. Expected YYYY-MM." });
    }
  ...
  fastify.get("/:slug/slots", async (request, reply) => {
    const { slug } = request.params as { slug: string };
    const { date: dateStr } = request.query as { date?: string };
   ...
   * No auth required.
   */
  fastify.post("/:slug/submit", async (request, reply) => {
    const { slug } = request.params as { slug: string };
    ...
    if (!body.customerName || body.customerName.trim().length < 2) {
      return reply.status(400).send({ message: "Customer name is required (min 2 characters)." });
    }
    ...
    if (!SERVICE_TYPES.includes(body.serviceType as any)) {
      return reply.status(400).send({ message: "Invalid service type." });
    }
    ...
    if (!body.preferredTime || !/^\d{2}:\d{2}$/.test(body.preferredTime)) {
      return reply.status(400).send({ message: "Invalid time format. Expected HH:MM." });
    }
   ...
   * No auth required.
   */
  fastify.get("/:slug/status/:bookingId", async (request, reply) => {
    const { slug, bookingId } = request.params as { slug: string; bookingId: string };
